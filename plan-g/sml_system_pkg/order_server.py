...
class OrderServer(Node):
    ...
    def _select_random_products(self) -> Tuple[List[int], List[int]]:
        ...
        # Advanced 단계 주문에는 Advanced 제품이 최소 하나 반드시 포함되어야 한다.
        if (
            self.tier == "advanced"
            and selected
            and not set(selected).intersection(ADVANCED_PRODUCT_IDS)
        ):
            selected[random.randrange(len(selected))] = random.choice(ADVANCED_PRODUCT_IDS)
