...
class PlannerCore:
    ...
    def build_plan(self, task):
        ...
        # 3. AMR-capable production, using assembly slots 5/6 and raw slides 0~4.
    ...
    def _append_amr_produce_batch(self, batch, station_info):
        ...
        for batch_index, po in enumerate(batch):
            ...
            # Base material goes directly to assembly slot 5 or 6.
    ...
    def _append_wb_produce(self, po, station_info):
        ...
        if load_step_ids:
            ...
            unload_sid = self._add_step(
                Step.AMR,
                Step.UNLOAD,
                [item['object_id'] for item in load_items],
                wb_id,
                unload_depends,
                [item['slide_id'] for item in load_items],
            )
        ...
        product_load_sid = self._add_step(
            Step.AMR,
            Step.LOAD,
            [po['product_id']],
            wb_id,
    ...
    def _append_recycle(self, ro, station_info):
        ...
        Scheduling intent:
          ...
          * Physical AMR product slots 5/6/9 are protected from double booking.
    ...
    def _choose_recycle_product_slot(self, ro):
        ...
        Small recycle products may use assembly slots 5/6.  Large recycle
        products use product slot 9.  The returned value is the physical slot
    ...
    def _append_recycle_interleaved_group(self, recycle_orders, station_info):
        ...
        Product preload rule:
          * first/preferred recycle product uses physical product slot 9;
    ...
    def _preload_recycle_products(self, jobs, start_index, loaded_queue, station_info,
        ...
        occupied_slots = {int(job['slot_index']) for job in loaded_queue if job.get('slot_index') is not None}
        selected = []
        selected_slots = []

        def reserve(job, slot_index):
        ...
        # First selected product always tries to use product slot 9.
        first = jobs[start_index]
        if PRODUCT_SLOT_INDEX not in occupied_slots:
            reserve(first, PRODUCT_SLOT_INDEX)
        elif self._can_use_recycle_aux_slot(first):
            aux = self._first_free_aux_recycle_slot(occupied_slots)
            if aux is None:
                return start_index, last_amr_step
            reserve(first, aux)
        else:
        ...
        anchor_counts = Counter(first['target_counts'])

        # Initial loading happens before WB starts, so be conservative.  During a
        # WB job, use time slack to decide whether extra preload can be done
        # without making WB wait for the next exchange.
        while next_index < len(jobs):
            ...
            if not self._can_use_recycle_aux_slot(candidate):
                break
            if self._target_overlap_count(anchor_counts, candidate['target_counts']) <= 0:
                break
            ...
            reserve(candidate, aux)
            anchor_counts.update(candidate['target_counts'])
            next_index += 1

        depends = [] if last_amr_step is None else [int(last_amr_step)]
        objects = [int(job['product_id']) for job in selected]
        slides = [int(job['slide_id']) for job in selected]
        load_sid = self._add_step(
            Step.AMR,
            Step.LOAD,
            objects,
    ...
    def _can_use_recycle_aux_slot(self, job):
        # Assembly slots 5/6 are reserved for additional preload.  Keep this
        # conservative: only small products whose decomposed raw size is 1-unit
        # materials may use 5/6.  Large products wait for product slot 9.
    ...
    def _append_recycle_raw_load_from_wb(self, job, station_info, depends_on):
        ...
        objects = [int(item['raw']) for item in items]
        slides = [int(item['slide_id']) for item in items]
        load_sid = self._add_step(
            Step.AMR,
            Step.LOAD,
            objects,
    ...
    def _append_recycle_raw_returns(self, job, station_info, depends_on, final_return=False):
        ...
        for target_station in station_order:
            ...
            unload_objects = [int(item['raw']) for item in group_items]
            unload_slides = [int(item['slide_id']) for item in group_items]
            deps = prev_deps if first else [int(last_unload)]
            last_unload = self._add_step(
                Step.AMR,
                Step.UNLOAD,
                unload_objects,
                int(target_station),
    ...
    def _append_deferred_waste_returns(self, station_info):
        ...
        3. Pack as many materials as possible into each AMR trip using raw slides 0~4
        ...
        for trip_index, trip_items in enumerate(trips):
            ...
            for target_station in station_order:
                ...
                unload_objects = [int(item['raw']) for item in items]
                unload_slides = [int(item['slide_id']) for item in items]
                prev_step = self._add_step(
                    Step.AMR,
                    Step.UNLOAD,
                    unload_objects,
                    int(target_station),
    ...
    def _append_grouped_load_steps(self, load_items):
        ...
        for station_id, items in self._order_grouped_by_travel(grouped).items():
            ...
            sid = self._add_step(
                Step.AMR,
                Step.LOAD,
                [int(item['object_id']) for item in items],
                station_id,
                depends,
                [int(item['slide_id']) for item in items],
            )
